...
class BenchmarkEngine(Engine):
    ...
    def update(self, dt: float) -> None:
        ...
        for moved_obj in objs_moved:
            ...
            for other_obj in self.game_objs.values():
...
def run_benchmark(num_objects: int, spacing: int = 50):
    pygame.init()
    screen = pygame.display.set_mode((800, 600))
    world_bounds = Rect(0, 0, 10000, 10000)
    engine = BenchmarkEngine(world_bounds)
    engine.screen = screen

    # Spawn dummy objects in a grid
    ...
    print(f"Benchmarking {num_objects} objects (brute-force)...")
    start = time.time()
    engine.update(0.016)
    end = time.time()
